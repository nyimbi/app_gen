...
def infer_page_structure(content, context):
    ...
    Expected JSON output includes:
      ...
      - For tender pages, include "tender_details": an object with keys: title, description,
        organization, issue_date, expiry_date, security_bond_required, location, activity,
        and optionally "documents" (list of URLs).
...
def discover_target_sites():
    """
    Uses SerpApi to exhaustively search the internet for African tender sites,
    INGO/NGO procurement portals, and multilateral agency tender sites.
    Queries are issued for various relevant keywords, and results are deduplicated by domain.
    ...
    queries = [
        ...
        "NGO tender Africa",
        "INGO tender Africa",
        "multilateral agency tender site Africa",
        "African procurement portal",
        "African government RFP"
    ]
    discovered_sites = {}
    ...
    for domain, url in discovered_sites.items():
        # Heuristically assign country:
        if "africa" in domain.lower():
            country = "Africa"
        else:
            country = "International"
        targets.append({"base_url": url, "country": country, "allow_external": True})
