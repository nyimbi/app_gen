...
class RFP(Base):
    ...
    procurement_method = Column(String, nullable=True)                # Open, restricted, negotiated, etc.
    submission_instructions = Column(Text, nullable=True)             # How to submit, format, etc.
    issue_date = Column(Date)
    prebid_date = Column(Date, nullable=True)                         # Pre-bid meeting date
    evaluation_date = Column(Date, nullable=True)                     # Evaluation period start/end
    award_date = Column(Date, nullable=True)                          # Award announcement date
    contract_commencement_date = Column(Date, nullable=True)          # Contract start date
    contract_duration = Column(String, nullable=True)                 # Duration or term of contract
    ...
    application_process = Column(Text, nullable=True)                 # Details on application and communication
...
def infer_page_structure(content, context):
    ...
    The output JSON now includes additional keys if available:
      - "tender_id", "scope", "contact_info", "eligibility_criteria",
        "procurement_method", "submission_instructions", "prebid_date",
        "evaluation_date", "award_date", "contract_commencement_date", "contract_duration",
        "estimated_contract_value", "legal_details", "sector", "subnational_location",
        "donor_info", "funding_program", "strategic_context", "application_process".
    Expected "page_type" remains one of ["list", "tender", "api", "landing", "search", "category",
...
- "page_type": one of ["list", "tender", "api", "landing", "search", "category", "document_repository",
  "announcement", "archived", "calendar", "guidelines", "api_documentation", "login", "redirect", "error",
  "multimedia", "comparison", "profile"].
- If the page is navigational (e.g. list, landing, search, category, document_repository, multimedia, comparison, profile, redirect, error, login), include "tender_links": a list of URLs.
- If the page is tender-like (tender, announcement, archived, calendar, guidelines, api_documentation), include "tender_details": an object with keys:
...
- If the page is "api", include "api_endpoint": URL.
Page Content:
{content}
    """
    try:
...
def discover_target_sites():
    ...
    (e.g. World Bank, UN agencies, UNDP, WHO, etc.).
...
def process_inference(url, inference, content_hash, meta, country, domain, allow_external=False):
    """
    Based on the inference result and expanded page types,
