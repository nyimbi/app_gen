...
class RFP(Base):
    ...
    tender_id = Column(String, unique=True, nullable=True)          # Unique tender/reference number
    title = Column(String)
    description = Column(Text)
    scope = Column(Text, nullable=True)                               # Detailed scope and objectives
    organization = Column(String)                                     # Issuing organization name
    contact_info = Column(Text, nullable=True)                        # Contact details for the issuing authority
    eligibility_criteria = Column(Text, nullable=True)                # Eligibility/qualification criteria
    procurement_method = Column(String, nullable=True)                # Procurement method (open, restricted, etc.)
    submission_instructions = Column(Text, nullable=True)             # Submission guidelines/instructions
    ...
    expiry_date = Column(Date)                                          # Submission deadline or expiry date
    estimated_contract_value = Column(Float, nullable=True)           # Budget or estimated value
    legal_details = Column(Text, nullable=True)                       # Contractual/legal conditions
    sector = Column(String, nullable=True)                            # Sector/industry classification
    subnational_location = Column(String, nullable=True)              # State, county, municipality, city
    donor_info = Column(Text, nullable=True)                          # Donor or funding agency info
    funding_program = Column(String, nullable=True)                   # Specific funding program
    strategic_context = Column(Text, nullable=True)                   # Development or strategic context
    application_process = Column(Text, nullable=True)                 # Application/communication process details
    security_bond_required = Column(Boolean)
    country = Column(String)
    location = Column(String)                                           # General location information
    activity = Column(String)                                           # Type of activity/service
    source_url = Column(String, unique=True)
    content_hash = Column(String, unique=True)
    is_worth_pursuing = Column(Boolean)                                 # Evaluation result
    documents = Column(Text)  # JSON encoded list of document URLs and extracted text
    metadata = Column(Text)   # JSON encoded additional meta information
...
def is_similar_duplicate(new_text, threshold=DUPLICATE_SIM_THRESHOLD):
    """
    Uses TF-IDF vectorization and cosine similarity to check if new_text is a near-duplicate
...
def parse_pdf_document(doc_url):
    """
    Downloads and extracts text from a PDF document.
    """
...
# ----------------------------- LLM CALL AND ITERATIVE PROMPT OPTIMIZATION -----------------------------
def call_llm(prompt: str) -> Optional[str]:
    """
    Calls the LLM via the Ollama API using the provided prompt.
...
def is_valid_json(text: str) -> bool:
    """
    Checks if the provided text is valid JSON.
    """
    try:
        json.loads(text)
        return True
...
def meets_quality(response_text: str, prompt: str) -> bool:
    """
    Heuristically checks if the response includes expected keys.
    For instance, we require a "page_type" key and, if tender-like, a "tender_details" key.
    """
    try:
        resp_json = json.loads(response_text)
        if "page_type" not in resp_json:
            return False
        if resp_json["page_type"] in ["tender", "announcement", "archived", "calendar", "guidelines", "api_documentation"]:
            if "tender_details" not in resp_json:
                return False
        return True
    except Exception:
        return False
...
def optimize_prompt(original_prompt: str, response_text: str) -> str:
    ...
    then provide an improved prompt that should extract the correct information.
    """
    optimization_prompt = f"""
You are an expert in prompt engineering. Compare the following prompt and the response you provided.
Original Prompt:
{original_prompt}
...
Identify any discrepancies or missing required information. Provide a revised version of the prompt that corrects these issues.
Return only the revised prompt.
    """
    optimized_response = call_llm(optimization_prompt)
...
def iterative_infer_page_structure(content: str, context: dict, max_iterations: int = 2) -> dict:
    """
    Iteratively obtains page classification and (if needed) detailed tender information.
    First, it asks for classification, then (if the page is tender-like) for details.
    After each call, it checks response quality and, if necessary, optimizes the prompt.
    """
    # Step 1: Classification
    classification_prompt = f"""
Based on the following page content, determine the page type from the following list:
...
Provide your answer in JSON format, for example:
...
Context: {context}
    """
    iteration = 0
    classification_response_text = call_llm(classification_prompt)
    ...
    # Step 2: If the page is tender-like, get detailed information.
...
Context: {context}
         """
         iteration = 0
         details_response_text = call_llm(details_prompt)
...
def discover_target_sites() -> list:
    """
    Uses SerpApi to exhaustively search the internet for tender sites by iterating
    over every African country and each administrative level (ministry, state, county,
    municipality, city) with tender/procurement-related keywords.
    
    In addition, it issues queries to capture NGO, INGO, and multilateral agency sites
    (e.g. World Bank, UN, UNDP, WHO, etc.).
    For each query, results are deduplicated by domain.
    Returns a list of target configurations.
    """
    african_countries = [
    ...
    discovered_sites = {}
    # Iterate over every combination of country, administrative level, and tender keyword.
    for country in african_countries:
        for admin_level in administrative_levels:
            for keyword in tender_keywords:
                query = f"{country} {admin_level} {keyword}"
                params = {
                    "api_key": SERPAPI_KEY,
                    "engine": "google",
                    "q": query,
    ...
    # Also search for international tender sites for NGO/INGO/multilateral agencies
    ...
    for domain, url in discovered_sites.items():
        assigned_country = "Africa"  # Default heuristic assignment; further refinement is possible.
        targets.append({"base_url": url, "country": assigned_country, "allow_external": True})
    logging.info(f"Discovered {len(targets)} candidate target sites from exhaustive queries.")
    return targets
...
def process_page(url: str, country: str, allow_external: bool = False):
    """
    Processes a single page: fetches the URL, infers page structure (using iterative LLM calls),
    ...
    # Use the iterative LLM inference function
...
def process_inference(url: str, inference: dict, content_hash: str, meta: dict, country: str, domain: str, allow_external: bool = False):
    """
    Based on the inference result and expanded page types, process the page accordingly:
      - For navigational pages, extract and enqueue tender links.
      - For tender-like pages, extract RFP details (including additional fields) and process documents.
...
def scrape_site(config: dict):
    """
    Scrapes a site using a breadth-first search (BFS) driven by iterative LLM inference and BS4 fallback.
...
def main_flow():
    """
    Main Prefect flow that first discovers target sites automatically via exhaustive queries,
    then concurrently scrapes each discovered site.
    """
    logger = get_run_logger()
    targets = discover_target_sites()
    if not targets:
        logger.error("No target sites discovered. Please check your search API configuration.")
        return
