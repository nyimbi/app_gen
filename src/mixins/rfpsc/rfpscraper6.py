...
def infer_page_structure(content, context):
    ...
    Expected JSON output includes:
      ...
      - For navigational pages, include "tender_links": a list of URLs.
      - For tender pages, include "tender_details": an object with keys: title, description,
        organization, issue_date (YYYY-MM-DD), expiry_date (YYYY-MM-DD),
        security_bond_required (boolean), location, activity, and optionally "documents" (list of URLs).
      - For API pages, include "api_endpoint": URL.
    """
    prompt = f"""
...
- If the page is navigational (e.g. list, landing, search, category, document_repository, multimedia, comparison, profile, redirect, error, login),
  include "tender_links": a list of URLs.
- If the page is tender-like (tender, announcement, archived, calendar, guidelines, api_documentation),
  include "tender_details": with keys: title, description, organization, issue_date (YYYY-MM-DD),
  expiry_date (YYYY-MM-DD), security_bond_required (boolean), location, activity, and optionally "documents" (list of URLs).
...
def discover_target_sites():
    ...
    for domain, url in discovered_sites.items():
        # Heuristically assign the country based on the query match.
        assigned_country = "Africa"  # default assignment; further refinement is possible
...
def process_inference(url, inference, content_hash, meta, country, domain, allow_external=False):
    ...
    process the page accordingly:
      - For navigational pages (list, landing, search, etc.), extract and enqueue tender links.
      - For tender-like pages, extract RFP details and process downloadable documents.
...
def main_flow():
    """
    Main Prefect flow that first discovers target sites automatically via search queries,
