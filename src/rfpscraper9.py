...
def iterative_infer_page_structure(
    content: str, context: dict, max_iterations: int = 2
) -> dict:
    """
...
Context: {context}
    """
    iteration = 0
    classification_response_text = call_llm(
        classification_prompt, model=LLM_MODELS["extraction"]
    )
    while iteration < max_iterations and (
        not is_valid_json(classification_response_text)
        or not meets_quality(classification_response_text, classification_prompt)
    ):
        classification_prompt = optimize_prompt(
            classification_prompt, classification_response_text
        )
        classification_response_text = call_llm(
            classification_prompt, model=LLM_MODELS["extraction"]
        )
...
Context: {context}
        ...
        while iteration < max_iterations and (not is_valid_json(details_response_text)):
            ...
            details_response_text = call_llm(
                details_prompt, model=LLM_MODELS["extraction"]
            )
            iteration += 1
        try:
...
def store_rfp(data: dict):
    ...
    try:
        rfp = RFP(
            ...
            evaluation_date=(
                tender.evaluation_date.date() if tender.evaluation_date else None
            ),
            award_date=tender.award_date.date() if tender.award_date else None,
            contract_commencement_date=(
                tender.contract_commencement_date.date()
                if tender.contract_commencement_date
                else None
            ),
...
def discover_target_sites() -> list:
            ...
            for keyword in tender_keywords:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logging.error(
                            f"Search query '{query}' failed with status {response.status_code}"
                        )
                except Exception as e:
    ...
    for keyword in international_keywords:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.error(
                    f"Search query '{keyword}' failed with status {response.status_code}"
                )
        except Exception as e:
    ...
    for domain, url in discovered_sites.items():
        assigned_country = (
            "Africa"  # heuristic; could be refined based on query context
        )
        targets.append(
            {"base_url": url, "country": assigned_country, "allow_external": True}
        )
...
def process_inference(
    ...
    if page_type in navigational_types:
        ...
        logger.info(
            f"Enqueueing {len(tender_links)} links from navigational page {url}"
        )
        ...
        if api_url:
            ...
            if api_response and api_response.status_code == 200:
                api_inference = iterative_infer_page_structure(
                    api_response.text, {"url": api_url, "country": country}
                )
                if api_inference and api_inference.get("page_type") in tender_like:
                    ...
                    if tender_data:
                        ...
                        tender_data["content_hash"] = hashlib.sha256(
                            api_response.text.encode()
                        ).hexdigest()
                        tender_data["documents"] = tender_data.get("documents", [])
...
def main_flow():
    ...
    logger.info(
        "Scraping completed. Please review sample records from the database for quality assurance."
    )
