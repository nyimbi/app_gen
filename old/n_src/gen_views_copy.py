...
from nx_widgets import TimePickerWidget, RatingWidget, RangeSliderWidget, RichTextEditorWidget,RelationshipGraphWidget,DurationWidget,DateRangePickerWidget,GeoPointWidget, MarkdownEditorWidget, MultiSelectWidget, FileUploadFieldWidget
...
"""
    ...
    for table_name in inspector.get_table_names():
        if table_name.lower().startswith(AB_PREFIX) or is_association_table(table_name, inspector):
            continue
...
def generate_model_view(table: sa.Table, inspector: sa.engine.reflection.Inspector, metadata: sa.MetaData) -> None:
    ...
    composite_key_info = handle_composite_keys(table)
...
def generate_custom_actions() -> str:
    ...
    from flask import flash, redirect, url_for, make_response, render_template, request
...
def generate_custom_actions_old() -> str:
    """
    *****Gradually incorporate these features**** #TODO
    Generate custom actions for the view class.
...
def get_list_columns(columns: List[str]) -> List[str]:
    ...
    return [col for col in columns if (col not in ignored_fields) and (not col.endswith(('_id_fk', '_img', '_audio', '_picture', '_blob', '_video')))]
    ...
    for fk in table.foreign_keys:
        if len(fk.column.foreign_keys) > 1:
            composite_info['foreign_keys'].append({
                ...
                'referred_columns': [fk.column.name for fk in fk.column.foreign_keys]
            })
...
def get_search_columns(table: sa.Table) -> List[str]:
    ...
    """
    return [col.name for col in table.columns if
        isinstance(col.type, (sa.String, sa.Text, sa.Unicode, sa.UnicodeText))
        and not col.name.endswith(('_img', '_audio', '_picture', '_blob', '_video'))
    ]

def get_search_form_query_rel_fields(table: sa.Table, metadata: sa.MetaData) -> Dict[str, Dict[str, Any]]:
    """
    ...
    for other_table in metadata.tables.values():
        if is_association_table(other_table, inspector):
            foreign_keys = [fk for fk in other_table.foreign_keys if fk.column.table.name != table.name]
            if table.name in [fk.column.table.name for fk in other_table.foreign_keys] and foreign_keys:
...
def get_form_extra_fields(table: sa.Table, session) -> Dict[str, Any]:
    ...
    for fk in table.foreign_keys:
        ...
        related_model_class = get_class_name(related_model.name, p)
...
def get_label_columns(table: sa.Table) -> Dict[str, str]:
    ...
    """
    return {col.name: snake_to_words(col.name) for col in table.columns if not col.name.endswith(('_img', '_audio', '_picture', '_blob', '_video'))}
...
def generate_validators(table: sa.Table) -> str:
    ...
    if validators:
        ...
        validator_strings = [f"'{column_name}': [{', '.join(column_validators)}]" for column_name, column_validators in validators.items()]
...
def get_related_tables(table: sa.Table, metadata: sa.MetaData, config: Dict[str, Any]) -> Set[sa.Table]:
    """
    ...
    if 'included_tables' in config:
        related_tables = {t for t in related_tables if t.name in config['included_tables']}
    if 'excluded_tables' in config:
        related_tables = {t for t in related_tables if t.name not in config['excluded_tables']}
...
def generate_multiple_views(table: sa.Table, metadata: sa.MetaData, config: Dict[str, Any]) -> None:
    """
...
def generate_main_query_and_schema(metadata):
    ...
    """
    query_classes = [f"{get_class_name(table.name, p)}Query" for table in metadata.tables.values()]
