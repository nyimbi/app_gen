...
def gen_models(metadata: MetaData, inspector: Any) -> List[str]:
    ...
    for table_name in inspector.get_table_names():
        ...
        if not is_association_table(table_name, inspector):
            ...
            table_code, reverse_rels_info = gen_table(table, inspector, relationship_info)
...
def gen_association_table(table_name, metadata, inspector):
    ...
    for fk in fks:
        ...
        relationship_name = local_col.replace('_id_fk', '').replace('_id', '')
        relationship_name = p.plural(relationship_name)

        relationship_str = f"relationship('{referred_class}', back_populates='{p.plural(table_name)}')"
        table_code.append(f"{INDENT}{relationship_name} = {relationship_str}")
...
def gen_table(table, inspector, relationship_info):
    ...
    for fk in fks:
        local_rel, reverse_rel_info = gen_relationship(fk, table_name, table_class, inspector, relationship_info)
        if local_rel:
...
def gen_column(column, pk_columns, fks, uqs, table_class):
    ...
    if column.get('default') is not None:
        default_value = process_default_value(column_name, column_type, column['default'])
        if default_value:
            attributes.append(f"default={default_value}")
    ...
    if is_enum_type(column_type, column.get('default')):
        ...
        # column_code.append(f"{enum_name} = Enum({enum_options}, name='{enum_name}')")
...
def gen_relationship(fk, table_name, table_class, inspector, relationship_info):
    ...
    # Add foreign_keys argument
    foreign_keys_str = f"[{', '.join([f'{col}' for col in fk_cols])}]"
    relationship_args = [
        f"'{referred_class}'",
        f"back_populates='{remote_relationship_name}'",
        f"foreign_keys={foreign_keys_str}"
    ]
    ...
    reverse_relationship_args = [
        ...
        f"foreign_keys='[{table_class}.{fk_cols[0]}]'"  # Assuming single-column FK for simplicity
    ]
...
def analyze_cardinality(table_name, fk, inspector, config=None):
    """Analyze the cardinality of a relationship with improved heuristics."""
    referred_table = fk["referred_table"]
    constrained_columns = fk["constrained_columns"]
    referred_columns = fk["referred_columns"]

    # Check for user-provided configuration
    if config and (table_name, referred_table) in config.get('relationships', {}):
        return config['relationships'][(table_name, referred_table)]
    ...
    if table_name == referred_table:
        relationship_type = analyze_self_referencing_relationship(table_name, constrained_columns, referred_columns, inspector)
        details = get_self_referencing_relationship_details(table_name, relationship_type, inspector)
    ...
    if len(constrained_columns) > 1:
        return analyze_composite_key_relationship(table_name, constrained_columns, inspector)

    # Analyze indexes for hints
    if has_unique_index_on_foreign_key(table_name, constrained_columns, inspector):
        return 'one-to-one'

    # Check naming conventions
    if follows_many_to_many_naming_convention(table_name, referred_table):
        return 'many-to-many'
