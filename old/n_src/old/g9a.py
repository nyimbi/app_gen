...
from nx_widgets import TimePickerWidget, RatingWidget, RangeSliderWidget, RichTextEditorWidget,RelationshipGraphWidget,DurationWidget,DateRangePickerWidget,GeoPointWidget, MarkdownEditorWidget, MultiSelectWidget
# Set up logging
import logging
...
def generate_views(db_uri: str) -> None:
    ...
    generated_code.append([
        "imports",
        f"""from flask_appbuilder import ModelView, MasterDetailView, MultipleView, ModelRestApi
...
from flask import flash, redirect, url_for, request, render_template, make_response, current_app
...
def get_search_columns(table: sa.Table) -> List[str]:
    ...
    """
    return [col.name for col in table.columns if isinstance(col.type, (sa.String, sa.Text, sa.Unicode, sa.UnicodeText))]
...
def generate_form_field(column: sa.Column, metadata: sa.MetaData) -> str:
    ...
    if isinstance(column.type, sa.String):
        return f"StringField('{label}', widget=BS3TextFieldWidget(), validators=[validators.DataRequired()])"
    elif isinstance(column.type, sa.Text):
        return f"TextAreaField('{label}', validators=[validators.DataRequired()])"
    elif isinstance(column.type, sa.Integer):
        return f"IntegerField('{label}', validators=[validators.DataRequired()])"
    elif isinstance(column.type, sa.Float):
        return f"FloatField('{label}', validators=[validators.DataRequired()])"
    elif isinstance(column.type, sa.Boolean):
        return f"BooleanField('{label}')"
    ...
    elif isinstance(column.type, sa.ForeignKey):
