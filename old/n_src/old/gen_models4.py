...
def gen_association_table(table_name, metadata, inspector):
    ...
    for fk in fks:
        ...
        remote_relationship_name= local_col.replace('_id_fk', '').replace('_id', '')
        local_relationship_name  = p.plural(table_name)

        relationship_str = f"relationship('{referred_class}', back_populates='{remote_relationship_name}')"
        table_code.append(f"{INDENT}{local_relationship_name} = {relationship_str}")
    ...
    for fk in fks:
        ...
        if not existing_relationship:
            ...
            relationship_str = (f"{INDENT}{relationship_name} = relationship('{other_table_class}', "
                                ...
                                f"back_populates='{relationship_name}')")
...
def gen_relationship(fk, table_name, table_class, inspector, relationship_info):
    ...
    if cardinality == 'many-to-many':
        ...
        if association_table:
            local_relationship_name = p.plural(referred_table.lower())
            remote_relationship_name = p.plural(table_name.lower())

            relationship_code.append(
                f"{INDENT}{local_relationship_name} = relationship('{referred_class}', "
                f"secondary='{association_table}', back_populates='{remote_relationship_name}')"
            )
            reverse_relationship_code.append(
                f"{remote_relationship_name} = relationship('{table_class}', "
                f"secondary='{association_table}', back_populates='{local_relationship_name}')"
            )
            processed_relationships.add(relationship_key)
            return relationship_code, reverse_relationship_code

    # Handle one-to-many and many-to-one relationships
    local_relationship_name = determine_relationship_name(fk_cols, table_name, referred_table, cardinality)
...
def determine_relationship_name(fk_cols, table_name, referred_table, cardinality):
    ...
    if base_name == referred_table.lower():
        ...
        return f"{base_name}_{referred_table.lower()}"
