...
from sqlalchemy import (
    String, Integer, Float, Boolean, DateTime, Date, Time, Numeric, Text
)
from sqlalchemy.dialects.postgresql import (
    UUID, ARRAY, JSON, JSONB, BYTEA, MONEY, INTERVAL, TIMESTAMP, ENUM,
    INET, MACADDR, CIDR, HSTORE, TSVECTOR, DOUBLE_PRECISION, REAL, BIGINT,
    SMALLINT
)
...
def generate_views(db_uri: str, config: Optional[Dict[str, Any]] = None) -> None:
    """
...
from flask_appbuilder.actions import action
from flask import session, flash, redirect, url_for, request, render_template, make_response, current_app, jsonify
...
def generate_model_view(
    ...
    - Handles composite primary keys and association tables
    - Lazy loading for related data
    - Caching for frequently accessed data
    - Pagination that adapts to primary key structure
    - Search optimization
    - Responsive design
    - Interactive filters
    ...
    Returns:
        None
    """
    class_name = snake_to_pascal(table.name, p)
    ...
    # Check if the table is an association table
    is_assoc = is_association_table(table.name, inspector)

    view_code = []

    # Generate basic attributes
    view_code.append(
        generate_basic_attributes(
            view_name, class_name, table, list_columns, pk_columns, inspector
        )
    )

    # Generate fieldsets
    # view_code.append(generate_fieldsets(field_sets))

    # # Generate templates and configurations
    # view_code.append(generate_templates_and_config())

    # # Generate exposed methods
    # view_code.append(generate_exposed_methods(pk_columns, composite_pk))

    # # Generate filters and search configurations
    # view_code.append(generate_filters_and_search(table, metadata, inspector))

    # # Generate form extra fields
    # view_code.append(generate_form_extra_fields(table, metadata))

    # # Generate validators
    # validators_code = generate_validators(table)
    # if validators_code.strip():
    #     view_code.append(validators_code)

    # # Generate custom actions if any
    # custom_actions_code = generate_custom_actions_code()
    # if custom_actions_code.strip():
    #     view_code.append(custom_actions_code)

    # # Generate lifecycle hooks if any
    # lifecycle_hooks_code = generate_lifecycle_hooks_code()
    # if lifecycle_hooks_code.strip():
    #     view_code.append(lifecycle_hooks_code)

    # Generate __repr__ method
    view_code.append(generate_repr_method_code(table))
...
def generate_basic_attributes(
    view_name, class_name, table, list_columns, pk_columns, inspector
):
    code = f"""
class {view_name}(ModelView):
    datamodel = SQLAInterface({class_name})
    endpoint = '{view_name.lower()}'
    list_title = '{snake_to_words(table.name)} List'
    show_title = '{snake_to_words(table.name)} Details'
    add_title = 'Add {snake_to_words(table.name)}'
    edit_title = 'Edit {snake_to_words(table.name)}'
    # list_columns = {list_columns}
    # show_columns = list_columns
    # edit_columns = [col for col in list_columns if col not in {pk_columns}]
    # add_columns = [col for col in list_columns if col not in {pk_columns}]
    # search_columns = {get_search_columns(table)}
    # search_exclude_columns = ['file', 'photo', 'image', 'cert_img', 'rt_img', 'audio', 'video', 'map']
...
def generate_fieldsets(field_sets):
    code = f"""
    # Field sets for add and edit forms
    add_fieldsets = {field_sets}
    edit_fieldsets = add_fieldsets
"""
    return code


def generate_templates_and_config():
    code = f"""
    # Template configurations
    # list_template = 'list_with_lazy_loading.html'"
    list_template = 'list_with_lazy_loading.html'
    edit_template = 'responsive_edit.html'
    show_template = 'responsive_show.html'
    add_template = 'responsive_add.html'
    ...
    page_size = 20
    base_order = ('{{pk_columns[0]}}', 'asc') if '{{pk_columns[0]}}' in list_columns else None
"""
    return code


def generate_exposed_methods(pk_columns, composite_pk):
    if not composite_pk and pk_columns[0] == "id":
        # Use keyset pagination for single primary key 'id'
        code = f"""
    @expose('/api/get_list')
    @has_access
    def api_get_list(self):
        ...
        if last_id:
            query = query.filter(self.datamodel.obj.id > last_id)
        if search:
            query = self.datamodel.query_search(query, search)

        items = query.order_by(self.datamodel.obj.id).limit(self.page_size).all()
...
"""
        ...
        # Use offset-based pagination for composite primary keys
        code = f"""
    @expose('/api/get_list')
    @has_access
    def api_get_list(self):
        ...
        items = query.order_by({', '.join([f'self.datamodel.obj.{col}' for col in pk_columns])}).offset((page - 1) * self.page_size).limit(self.page_size).all()
        data = [{{col: getattr(item, col) for col in self.list_columns}} for item in items]
        has_more = len(items) == self.page_size
...
def generate_filters_and_search(table, metadata, inspector):
    code = f"""
    # Search configuration
    search_form_query_rel_fields = {get_search_form_query_rel_fields(table, metadata, inspector)}
    filter_rel_fields = {get_filter_rel_fields(table, metadata, inspector)}
    filter_exclude_columns = ['file', 'photo', 'image']
    filter_template = 'interactive_filters.html'
    base_filters = []
    ...
    def filter(self):
        ...
        for col, value in filter_params.items():
            if col in self.search_columns and value:
    ...
    def search(self):
        ...
        if search_query:
            query = self.datamodel.session.query(self.datamodel.obj)
            search_filter = [getattr(self.datamodel.obj, col).ilike(f'%{{search_query}}%') for col in self.search_columns]
...
def generate_form_extra_fields(table, metadata):
    form_fields = generate_form_fields(table, metadata)
...
def generate_custom_actions_code():
    custom_actions_code = generate_custom_actions()
    if custom_actions_code.strip():
        code = f"""
    {custom_actions_code}
"""
...
def generate_lifecycle_hooks_code():
    lifecycle_hooks_code = generate_lifecycle_hooks()
    if lifecycle_hooks_code.strip():
        code = f"""
    {lifecycle_hooks_code}
"""
    else:
        code = ""
    return code
...
def generate_repr_method_code(table):
    repr_method_code = generate_repr_method(table)
    code = f"""
    {repr_method_code}
"""
    return code
...
def generate_custom_actions() -> str:
    ...
    # Custom actions
    @action("muldelete", "Delete", "Delete selected items?", "fa-trash", multiple=True)
    def muldelete(self, items):
        self.datamodel.delete_all(items)
        flash(f"Deleted {{len(items)}} item(s)", "success")
        return redirect(self.get_redirect())

    @action("export_csv", "Export CSV", "Export selected items to CSV?", "fa-file-excel-o", multiple=True)
    def export_csv(self, items):
        if not isinstance(items, list):
            items = [items]
        si = io.StringIO()
        cw = csv.writer(si)
        cw.writerow(self.list_columns)
        for item in items:
            cw.writerow([getattr(item, col) for col in self.list_columns])
        output = make_response(si.getvalue())
        output.headers["Content-Disposition"] = f"attachment; filename={{self.__class__.__name__}}.csv"
    ...
    @action("bulk_edit", "Bulk Edit", "Edit selected items?", "fa-edit", multiple=True)
    def bulk_edit(self, items):
        ids = [str(getattr(item, self.datamodel.get_pk_name())) for item in items]
        return redirect(url_for(f'{{self.__class__.__name__}}.bulk_edit_form', ids=','.join(ids)))
    ...
    def bulk_edit_form(self, ids):
        ids = ids.split(',')
        pk_name = self.datamodel.get_pk_name()
        items = self.datamodel.session.query(self.datamodel.obj).filter(getattr(self.datamodel.obj, pk_name).in_(ids)).all()
        if not items:
            flash("No items selected for bulk edit.", "warning")
            return redirect(self.get_redirect())

        form = self.bulk_edit_form(refresh=True)
        if form.validate_on_submit():
            for item in items:
                for field_name, field_value in form.data.items():
                    if field_name in self.edit_columns and field_value is not None:
                        setattr(item, field_name, field_value)
            self.datamodel.session.commit()
            flash(f"Successfully updated {{len(items)}} item(s).", "success")
            return redirect(self.get_redirect())
...
######
def generate_custom_actions_old() -> str:
    """
    Generate custom actions for the view class.
...
def handle_composite_keys(table: sa.Table) -> Dict[str, Any]:
    """
    Handle composite primary and foreign keys for a table.

    Args:
        table (sa.Table): SQLAlchemy Table object

    Returns:
        Dict[str, Any]: Dictionary containing information about composite keys
    """
    composite_info = {"primary_key": [], "foreign_keys": []}

    # Handle composite primary key
    if len(table.primary_key.columns) > 1:
        composite_info["primary_key"] = [col.name for col in table.primary_key.columns]

    # Handle composite foreign keys
    for fk in table.foreign_keys:
        if len(fk.column.foreign_keys) > 1:
            composite_info["foreign_keys"].append(
                {
                    "constrained_columns": [col.name for col in fk.column.foreign_keys],
                    "referred_table": fk.column.table.name,
                    "referred_columns": [
                        fk.column.name for fk in fk.column.foreign_keys
                    ],
                }
            )
...
def get_search_form_query_rel_fields(
    table: sa.Table, metadata: sa.MetaData, inspector
    ...
    for fk in table.foreign_keys:
        ...
        parent_class_name = snake_to_pascal(parent_table.name, p)
    ...
    for other_table in metadata.tables.values():
        if is_association_table(other_table.name, inspector):
            foreign_keys = [
                fk
            ...
            if (
                ...
                other_class_name = snake_to_pascal(other_table_name, p)
...
def get_form_extra_fields(table: sa.Table, session) -> Dict[str, Any]:
    ...
    for fk in table.foreign_keys:
        ...
        related_model_class = snake_to_pascal(related_model.name, p)
...
def generate_form_query_rel_fields(table: sa.Table, metadata: sa.MetaData) -> str:
    ...
    for fk in table.foreign_keys:
        ...
        referred_class = snake_to_pascal(referred_table.name, p)
...
def generate_form_field(column: sa.Column, metadata: sa.MetaData) -> str:
    ...
    # Handle image columns (ending with '_img')
    if field_name.endswith('_img'):
        return ''
        # return f"FileField('{label}')"

    # Handle file columns (ending with '_file')
    ...
    if isinstance(column.type, String):
        ...
        related_model = snake_to_pascal(related_table.name, p)
...
def generate_validators(table: sa.Table) -> str:
    """
    Generate validators for table columns, handling all PostgreSQL data types.
    ...
    """
    # Import necessary types from SQLAlchemy and PostgreSQL dialects

    validators_dict = {}
    for column in table.columns:
        column_validators = []

        # Not nullable and not primary key
        if not column.nullable and not column.primary_key:
            column_validators.append("InputRequired()")
        else:
            column_validators.append("Optional()")

        # Map SQLAlchemy types to WTForms validators
        col_type = column.type

        # Handle String types
        if isinstance(col_type, (String, Text)):
            if hasattr(col_type, "length") and col_type.length:
                column_validators.append(f"Length(max={col_type.length})")

        # Handle Integer types
        elif isinstance(col_type, (Integer, BIGINT, SMALLINT)):
            column_validators.append("NumberRange(min=-2147483648, max=2147483647)")

        # Handle Numeric and Float types
        elif isinstance(col_type, (Float, Numeric, DOUBLE_PRECISION, REAL, MONEY)):
            column_validators.append("NumberRange(min=-1e308, max=1e308)")

        # Handle Boolean types
        elif isinstance(col_type, Boolean):
            pass  # No additional validators needed

        # Handle Date and Time types
        elif isinstance(col_type, DateTime):
            column_validators.append("DataRequired()")
        elif isinstance(col_type, Date):
            column_validators.append("DataRequired()")
        elif isinstance(col_type, Time):
            column_validators.append("DataRequired()")
        elif isinstance(col_type, INTERVAL):
            pass  # No specific validator in WTForms

        # Handle UUID types
        elif isinstance(col_type, UUID):
            column_validators.append("UUID()")  # Custom validator needed

        # Handle ENUM types
        elif isinstance(col_type, ENUM):
            enum_values = [f"'{e}'" for e in col_type.enums]
            enum_values_str = ", ".join(enum_values)
            column_validators.append(f"AnyOf([{enum_values_str}])")

        # Handle JSON and JSONB types
        elif isinstance(col_type, (JSON, JSONB)):
            pass  # Custom validator may be needed

        # Handle ARRAY types
        elif isinstance(col_type, ARRAY):
            pass  # Custom handling may be needed

        # Handle BYTEA (binary) types
        elif isinstance(col_type, BYTEA):
            pass  # No specific validator

        # Handle Network address types
        elif isinstance(col_type, (INET, CIDR)):
            column_validators.append("IPAddress()")
        elif isinstance(col_type, MACADDR):
            column_validators.append("MACAddress()")  # Custom validator needed

        # Handle HSTORE types
        elif isinstance(col_type, HSTORE):
            pass  # Custom handling may be needed

        # Handle TSVECTOR types
        elif isinstance(col_type, TSVECTOR):
            pass  # Custom handling may be needed

        # Unhandled types
        else:
            pass  # No validators added

        if column_validators:
            validators_dict[column.name] = column_validators

    if validators_dict:
        # Construct the validators_columns dictionary as a string
        validator_strings = []
        for column_name, column_validators in validators_dict.items():
            validators_list = ", ".join(column_validators)
            validator_strings.append(f"    '{column_name}': [{validators_list}]")
        validators_dict_str = ",\n".join(validator_strings)
        code = f"""
    validators_columns = {{
        {validators_dict_str}
        }}
...
def generate_master_detail_views(table: sa.Table, metadata: sa.MetaData) -> None:
    ...
    for fk in table.foreign_keys:
        ...
        parent_class_name = snake_to_pascal(parent_table.name, p)
        child_class_name = snake_to_pascal(table.name, p)
...
def generate_wizard_view(table: sa.Table) -> None:
    ...
    # Begin building the Wizard View code
    wizard_code = f"""
class {{view_name}}(BaseView):
    ...
    @has_access
    def list(self):
        return redirect(url_for(f'.step_view', step=1))
   ...
   def render_wizard(self, step, form, step_description):
    ...
    def back(self, step):
        if step > 1:
            return redirect(url_for(f'.step_view', step=step-1))
        return redirect(url_for('.list'))
    """
    ...
    for step in range(1, total_steps + 1):
        ...
        # Generate step view code
        wizard_code += f"""
    @expose('/step<int:step>', methods=['GET', 'POST'])
    @has_access
    @permission_name('step')
    def step_view(self, step):
        ...
        end_idx = min(step * 5, len(columns))
        step_columns = columns[start_idx:end_idx]
        step_description = f"Step {{step}}: {{', '.join(step_columns)}}"

        {generate_form_fields_for_wizard(columns, table)}
        ...
        return self.render_wizard(step, form, step_description)
        """
    ...
    def submit(self):
        if all(self.is_step_complete(step) for step in range(1, self.total_steps + 1)):
            ...
            return redirect(url_for('.step_view', step=1))
        else:
...
#            previous_step=step - 1 if step > 1 else None,
#            next_step=step + 1 if step < self.total_steps else 'submit',
#            modelview_name=self.__class__.__name__,  # Add this line
#        )
...
#         # Pre-fill form with previously saved data if available
#         form_data = self.get_form_data(step)
#         for field in form:
#             if field.name in form_data:
#                 field.data = form_data[field.name]

#         # Render the wizard step with description
#         step_description = f"Step {{step}}: {{', '.join(step_columns)}}"
#         return self.render_wizard(step, form, step_description)
#         """
...
def generate_main_query_and_schema(metadata):
    ...
    """
    query_classes = [
        f"{snake_to_pascal(table.name, p)}Query" for table in metadata.tables.values()
    ]
...
if __name__ == "__main__":
    ...
    write_templates(os.path.join(output_dir, "templates"))
