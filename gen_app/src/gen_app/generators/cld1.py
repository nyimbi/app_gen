...
class LLMClient:
    """Client for LLM API interactions with advanced features"""
    
    @staticmethod
    async def generate_async(prompt: str, context: str, 
                           max_retries: int = None,
                           temperature: float = None) -> str:
        """Asynchronous generation with retries and error handling"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Store in cache
                GenerationCache.store(prompt, context, result)
    ...
    def generate(prompt: str, context: str, 
        ...
        return asyncio.run(LLMClient.generate_async(prompt, context, max_retries, temperature))
...
class ProjectGenerator:
    ...
    async def generate_component_async(self, component: Component, file_path: str) -> str:
        ...
        with console.status(f"Generating {component.type} '{component.name}'..."):
            ...
            code = await LLMClient.generate_async(prompt, file_path)
            ...
            if Config.options.verify_syntax:
                ...
                if not valid:
                    ...
                    # We could regenerate here, but for now just log the warning
    ...
    async def generate_test_async(self, component: Component, file_path: str, code: str) -> str:
        ...
        with console.status(f"Generating tests for '{component.name}'..."):
            ...
            return await LLMClient.generate_async(prompt, file_path)
    ...
    async def plan_development_async(self, description: str) -> Dict:
        ...
        with console.status("Planning development..."):
            ...
            plan_json = await LLMClient.generate_async(prompt, description)
            ...
            try:
                ...
                raise GenerationError("Invalid plan JSON format") from e
    ...
    async def create_directory_structure_async(self, plan: Dict) -> Structure:
        ...
        with console.status("Creating directory structure..."):
            ...
            structure_json = await LLMClient.generate_async(prompt, json.dumps(plan))
            ...
            try:
                ...
                raise GenerationError("Invalid structure JSON format") from e
