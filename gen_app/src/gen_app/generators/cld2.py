...
class OutputParser:
    ...
    def extract_code(raw_output: str) -> str:
        ...
        # Filter out potential non-code explanatory lines
        # (lines that look like markdown headings, bullet points, or explanatory text)
        non_code_patterns = [
            r"^#\s+", # Markdown headings
            r"^\*\s+", # Bullet points
            r"^-\s+", # Bullet points
            r"^>", # Blockquotes
            r"^Here's", # Common explanatory starts
            r"^This", 
            r"^The",
            r"^Note:",
            r"^Explanation:"
        ...
        def is_likely_code(line):
            ...
            # Check if line matches any non-code pattern
            if any(re.match(pattern, line.strip()) for pattern in non_code_patterns):
                return False
                
            # Look for code indicators (imports, class/function definitions, etc.)
            code_indicators = [
                r"^import\s+", 
                r"^from\s+.*\s+import\s+",
                r"^def\s+", 
                ...
                r"^@", # Decorator
                r"^if\s+__name__\s+==", 
                r"=[^=]", # Assignment but not comparison
                r"return\s+",
                r"^\s*for\s+.*\s+in\s+",
                r"^\s*while\s+"
            ]
            ...
            # Default behavior - include lines we're not sure about
            return True
        
        code_lines = [line for line in lines if is_likely_code(line)]
        
        # If we filtered more than half the lines, we might have explanatory text mixed with code
        if len(code_lines) < len(lines) / 2:
            logger.warning("Output appears to contain significant non-code content")
